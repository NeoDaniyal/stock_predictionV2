...
import numpy as np
import pandas as pd
from sklearn.metrics import (accuracy_score, balanced_accuracy_score, classification_report, f1_score, log_loss, precision_score, recall_score)
import os
...
def load_oof_predictions():
    """Load all model out-of-fold prediction CSV files."""
...
def align_predictions(oof_data):
    """Align OOF Predictions from all models on (Date, Ticker)."""
    base_m = "xgboost"
    ...
    for m, df in oof_data.items():
        ...
        base_df = pd.merge(base_df, probs_df, on=["Date", "Ticker"], how="inner")
...
def compare_ensembles():
    ...
    if len(oof_data) < 2:
        print("Not enogh OOF prediction files found to run ensemble evaluations.")
        return
    merged_df = align_predictions(oof_data)
    print(f"Align {len(merged_df)} common out-of-fold samples for ensemble evaluation.")
    ...
    comp_df = pd.DataFrame([metrics_4, metrics_5], index=["4-Model Ensemble (Trees)", "5-Model Ensemble (Tree + LSTM)"])
