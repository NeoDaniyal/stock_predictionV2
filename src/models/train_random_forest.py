...
PREDICTION_PATH = Path("reports/random_forest")
...
def train_model(model, X_train, y_train):
    ...
    Internal class_weight="balanced" handles class imbalance during tree splits.
    """
...
def train_and_evaluate():
    ...
    for year, train_df, test_df in folds:
        ...
        # Compute & print fold class distribution for logging
    ...
    # Calculate Feature Importance Mean & Std
    ...
    return last_model, predictions_df, metrics_df, avg_importance, df


def save_reports(predictions_df, metrics_df, avg_importance, df):
    PREDICTION_PATH.mkdir(parents=True, exist_ok=True)

    oof_path = PREDICTION_PATH / "rf_oof_predictions.csv"
    predictions_df.to_csv(oof_path, index=False)
    print(f"\nOOF predictions saved to: {oof_path}")

    metrics_path = PREDICTION_PATH / "rf_walk_forward_metrics.csv"
    metrics_df.to_csv(metrics_path, index=False)
    print(f"Walk-forward metrics saved to: {metrics_path}")

    importance_path = PREDICTION_PATH / "rf_average_feature_importance.csv"
    avg_importance.to_csv(importance_path)
    print(f"Average feature importance saved to: {importance_path}")

    # Save experiment summary metadata JSON
    summary_metadata = {
        "Model": "Random Forest",
...
def save_model(model):
    ...
    path = MODEL_PATH / "random_forest_v1.pkl"
    joblib.dump(model, path)
    print(f"Final model saved to: {path}")


def main():
    model, predictions_df, metrics_df, avg_importance, raw_df = (
        train_and_evaluate()
    )

    print("\n" + "=" * 60)
    print("Walk Forward Summary (Random Forest):")
    print(metrics_df)
    print("\nAverage Metrics:")
    ...
    save_reports(predictions_df, metrics_df, avg_importance, raw_df)
    save_model(model)
