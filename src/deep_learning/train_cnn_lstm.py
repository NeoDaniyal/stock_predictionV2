...
class CNNLSTMModel(nn.Module):
    def __init__(self, input_dim, conv_channels=32, kernel_size=3, lstm_hidden=64, num_classes=3, dropout=0.2):
        super().__init__()
        # Conv1D expects shape: (Batch, Channels/Features, Seq_Len)
        ...
        # LSTM processes sequence of feature channels: (Batch, Seq_Len, Conv_Channels)
        self.lstm = nn.LSTM(
    ...
    def forward(self, x):
        # x shape: (Batch, Seq_Len, Features) -> Permute for Conv1D: (Batch, Features, Seq_Len)
        x = x.permute(0, 2, 1)
        x = self.conv1d(x)
        x = self.bn(x)
        x = self.relu(x)
        ...
        # Permute back for LSTM: (Batch, Seq_Len, Conv_Channels)
        x = x.permute(0, 2, 1)
        out, _ = self.lstm(x)
        
        # Extract last sequence output
...
def train_and_eval_cnn_lstm():
    ...
    # Leakage-free feature set exclusion
    ...
    print(f"Training CNN-LSTM with {len(feature_cols)} features over 30-day windows.")
    ...
    for test_year in walk_forward_years:
        print(f"\n=================== TRAINING CNN-LSTM FOR TEST YEAR {test_year} ===================")
        ...
        # Strict probability normalization
    ...
    oof_output_path = REPORTS_DIR / "cnn_lstm_oof_predictions.csv"
    full_oof_df.to_csv(oof_output_path, index=False)

    summary_df = pd.DataFrame(fold_summaries)
    print("\n=================== CNN-LSTM WALK-FORWARD SUMMARY ===================")
    ...
    print(f"\n✅ CNN-LSTM training complete! Total OOF rows generated: {len(full_oof_df):,}")
    print(f"   OOF predictions saved to: {oof_output_path}")


if __name__ == "__main__":
    train_and_eval_cnn_lstm()