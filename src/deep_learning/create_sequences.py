...
FEATURE_DATA_PATH = Path("data/features/final_dataset.csv")
OUTPUT_DIR = Path("data/deep_learning")
...
def create_sequences_by_ticker(df_subset, feature_cols, sequence_length=30):
    """Creates sliding window 3D arrays (samples, sequence_length, features) per ticker."""
    X_seqs = []
    y_targets = []
    meta_records = []
    ...
    for ticker, group in df_subset.groupby("Ticker"):
        ...
        if len(X_seqs) == 0:
            return(
                np.array([]),
                np.array([]),
                pd.DateaFrame(columns=["Date", "Ticker", "Target", "Year"])
            )
        return(
...
def build_walk_farward_datasets(sequence_length=30):
    """Generate fold-wise scaled train/test 3D sequences metrics for walk-farward evaluation."""
    ...
    for test_year in test_years:
        ...
        X_train, y_train, meta_train = create_sequences_by_ticker(train_df, feature_cols, sequence_length)
        X_test, y_test, meta_test = create_sequences_by_ticker(test_df, feature_cols, sequence_length)
        ...
        # Save numpy binary arrays for fast PyTorch / TensorFlow loading
    ...
    joblib.dump(test_years, OUTPUT_DIR / "feature_names.pkl")
    print(f"\nAll walk-forward sequences created and save successfully to: {OUTPUT_DIR.resolve()}")

if __name__ == "__main__":
    build_walk_farward_datasets(sequence_length=SEQUENCE_LENGTH)
